...
%     yi(y(i))=1;
%     for k=1:num_labels
%         JTheta(i,k)=yi(k)*log(a3(k))+(1-yi(k))*log(1-a3(k));
%     end
%     delta3=a3-yi;
%     delta2=Theta2(:,2:end)'*delta3.*(a2.*(1-a2)); %sigmoid derivate
%     Theta1_grad=Theta1_grad+delta2*[1;a1]';
%     Theta2_grad=Theta2_grad+delta3*[1;a2]';
% end
% J=-sum(sum(JTheta))/m+lambda/2/m*(sum(sum(Theta1(:,2:end).^2))+sum(sum(Theta2(:,2:end).^2)));
...
Theta1_grad=Delta2*[ones(1,size(A1,2));A1]'/m+lambda/m*[zeros(size(Theta1,1),1) Theta1(:,2:end)];
Theta2_grad=Delta3*[ones(1,size(A2,2));A2]'/m+lambda/m*[zeros(size(Theta2,1),1) Theta2(:,2:end)];
